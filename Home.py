...
with col3:
    link = "pages/3_생각+_(이미지_생성).py"
    st.markdown('<div style="background-color:#90EE90; padding: 10px; border-radius: 10px; text-align:center;">', unsafe.allow_html=True)
    st.markdown('<h3 style="text-align:center;">생각+</h3>', unsafe.allow_html=True)
    st.markdown('<p style="text.align:center;">이미지 생성    <br> <span style="color:rgba(0,0,0,0);"> 이미지 생성 </span> </p>', unsafe.allow_html=True)  # 공백을 추가하여 두 줄로 표현
