...
situation1 = st.radio(
    "1_친구들과 공원에서 놀다가 음료수를 사서 마셨습니다. 다 마셨는데 공원에는 쓰레기통도 보이지 않고 나에게는 가방도 없습니다. 음료수를 먹고 난 페트병을 어떻게 할까요?",
...
situation2 = st.radio(
    "2_처음으로 친구들과 함께 극장에 갔습니다. 너무 신이 나고 즐거워서 영화를 보면서 입꼬리가 내려가질 않습니다. 영화를 보면서 친구에게 영화 내용을 말하고 싶은데 어떻게 할까요?",
...
situation3 = st.radio(
    "3_오늘은 근처 공원에서 축제가 열리는 날입니다. 선착순 100명에게는 내가 요즘 정말 좋아하는 캐릭터 장난감을 준다고 합니다. 줄을 서 있지만 선착순 100명 안에는 들지 못할 것 같아서 너무 속상합니다. 그런데 기다리다가 화장실을 다녀오며 보니 줄의 가운데가 약간 비어 있고 뒤에 있는 사람들은 무언가에 정신이 팔려 있습니다. 이럴 때 나는 어떻게 할까요?",
...
def analyze_moral_data(name, responses, situation1, situation2, situation3, thoughts, total_score):
    ...
    사용자가 제공한 도덕적 상황, 겪은 상황, 판단, 느낌, 행동 등을 기반으로 도덕적 피드백을 작성합니다. 양식은 <결과> 인성분석 결과는 종합적으로 결과를 판단해서 최대한 상세하게 이야기 해준다. <피드백> 결과에 대한 피드백으로 제공합니다.
    다음은 사용자가 제공한 내용입니다:
    이름: {name}
    응답: {responses}
    상황1: {situation1}
