...
def generate_story(virtue, story):
    persona = f'''
    이 프롬프트는 사용자로부터 제공된 가치덕목과 인성 스토리를 바탕으로 인성 광고 영상 스토리를 제작하는 GPT 모델입니다. 스토리를 입력하면 동영상에 대한 묘사를 해주세요. 아주 짧은 형식의 광고영상을 만들겁니다. 사용자가 제공한 내용이 현실에 맞지 않거나 사실에 맞느 않은 경우 gtp가 수정해서 작성합니다. 인성광고스토리를 작성한 다음 https://lumalabs.ai/dream-machine 사이트를 이용하도록 추천해주세요.
    다음은 사용자가 제공한 내용입니다:
    가치덕목: {virtue}
    인성 스토리: {story}
    '''
